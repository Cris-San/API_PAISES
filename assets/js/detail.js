...
async function getDatos(codigo){
    ...
    try {
        ...
        if (pais.length > 0) {
            ...
            document.getElementById('country-language').textContent = Object.values(data.
        languages || { '': 'No Disponible' }).join(', ');
            document.getElementById('country-currency').textContent = data.currencies ? Object.
        values(data.currencies)[0].name : 'No Disponible';
            document.getElementById('country-flag').src = data.flags.svg;
            document.getElementById('country-coat').src = data.coatOfArms.svg;
            const latlng = data.latlng;
            const mapURL = `https://www.openstreetmap.org/export/embed.html?bbox=${latlng[1] - 
        10},${latlng[0] - 10},${latlng[1] + 10},${latlng[0] + 10}&layer=mapnik`;
